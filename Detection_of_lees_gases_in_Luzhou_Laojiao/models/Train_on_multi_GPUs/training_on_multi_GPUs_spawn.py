...
def main(args, rank, world_size, dataset, model, criterion1, criterion2):
    ...
    # optimizer
    # pg = [p for p in model.parameters() if p.requires_grad]  以后如果做fine-tune用得上
...
if __name__ == "__main__":
    ...
    # args, rank, world_size, dataset, model, criterion1, criterion2

    world_size = opt.world_size
    processes = []
    for rank in range(world_size):
        p = Process(target=main, args=(opt, rank, world_size, train_data_set, spectrans, criterion1, criterion2))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()
